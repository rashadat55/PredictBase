...
// ✅ SIMPLE FIX: Non-blocking SDK init
let ENV = 'BASE_APP';
...
async function init() {
    try {
        ...
        // Silent fail - just use BASE_APP env
        ENV = 'BASE_APP';
    }
}
    ...
    list.innerHTML = filtered.map(m => `
        <div class="card p-4 flex flex-col gap-3">
            <div class="flex justify-between items-start text-[8px] font-bold text-gray-500 uppercase tracking-widest">
                ...
                <div class="text-green-500">${m.date}</div>
            </div>
...
// ✅ KEY: Render matches FIRST, init SDK in background (non-blocking)
render();
init();
